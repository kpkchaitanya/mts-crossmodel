# M0-M8 Product and Feature End-to-End Tracker
...
3. Worked example for the current MTS Weekly Math Worksheet effort
...
**Required artifact set:** Run manifest, resolved configuration snapshot, input/output artifacts, approvals, telemetry, logs/metrics, staging or production QA, user feedback, incidents, support notes, and operational decision record.
...
## MTS Weekly Math Worksheet Worked Example

This is an example of how the reusable tracker can be populated. It is not the canonical definition of M0-M8.
...
| M7 | In progress | Current weekly run is staged in Google Docs; human formatting review and operational disposition remain. |
| M8 | Not complete | Governance, learning capture, final approval, and optimization review remain after the run is resolved. |
...
6. Keep run artifacts under the run directory and link them here rather than copying them into the tracker.

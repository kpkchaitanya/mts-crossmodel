"""Prepare Gate 1 scope review data for Math Weekly Worksheets."""
from __future__ import annotations

from collections.abc import Mapping
from typing import Any

from mts.subjects.math.subject_module import MathSubjectModule
...
def prepare_scope_review(
	...
	*,
	...
	if effective_config.get("subject") != "math":
		raise WeeklyWorkflowError("Math Weekly workflow requires a Math effective config.")
	if effective_config.get("worksheet_type_id") != "weekly-worksheet":
	...
	grade_catalog = subject_module.module_root / str(catalog["grade_course_catalog"])
	grade_entries = subject_module_data(grade_catalog).get("grades_and_courses", [])
	...
	for entry in grade_entries:
		...
		if grade_id == "grade_9_10":
			plans.append(_combined_high_school_plan(entry, on_date, effective_config, subject_module))
		else:
			scope = subject_module.resolve_curriculum({"grade_or_course": grade_id, "on_date": on_date})
			plan = subject_module.prepare_blueprint(scope, effective_config, effective_config)
...
def _combined_high_school_plan(
	...
	plan = subject_module.prepare_blueprint(combined_scope, effective_config, effective_config)
	...
	plan["grade_split"] = dict(effective_config["grade_defaults"][entry["id"]]["grade_split"])
	...
	if plan["questions_per_week"] != plan["questions_per_day"] * len(effective_config["sections"]):
...
def subject_module_data(path) -> dict[str, Any]:
	import json

	return json.loads(path.read_text(encoding="utf-8"))


__all__ = ["WeeklyWorkflowError", "prepare_scope_review"]

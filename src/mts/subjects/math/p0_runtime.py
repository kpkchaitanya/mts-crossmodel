...
def standards_context(standards: dict[str, Any], grade: str, current: list[str]) -> dict[str, Any] | None:
    """Return cached NC standard domains matching a resolved weekly scope."""
    grade_data = standards.get("grades", {}).get(grade)
    if not isinstance(grade_data, dict):
        return None
    matching_domains = []
    for domain_id, domain in grade_data.get("domains", {}).items():
        domain_standards = domain.get("standards", [])
        if any(
            isinstance(standard, str)
            and any(standard == target or standard.startswith(f"{target}.") for target in current)
            for standard in domain_standards
        ):
            matching_domains.append({
                "id": domain_id,
                "standards": domain_standards,
                "topics": domain.get("topics", []),
            })
    if not matching_domains:
        return None
    return {
        "source": "nc_standards_cache",
        "source_url": grade_data.get("source"),
...
def resolve_curriculum(
    ...
    """Resolve cached weekly scope with optional progression and standards context."""
    ...
    if standards is not None:
        ctx = standards_context(standards, grade, out.get("current", []))
        if ctx is not None:
            out["standards_context"] = ctx
            if out.get("source") == "month_cache" and not out.get("topics"):
                out["topics"] = [
                    topic
                    for domain in ctx["domains"]
                    for topic in domain["topics"]
                ]
                out["topics_source"] = "nc_standards_cache"
    return out
...
def compute(method: str, inputs: dict[str, Any]) -> Any:
    ...
    if method == "ratio":
        return f"{inputs['left']}:{inputs['right']}"

...
```powershell
python scripts/generate_worksheet.py subject=math worksheettype=weekly week=next gates="bypass all"
```

The CLI accepts the same `key=value` parameters described below. In non-interactive CLI mode, gate
bypass must be explicit (`gates="bypass all"` or `gates="bypass <gate_id>[,<gate_id>...]"`); `gates=all`
is refused because there is no interactive approval prompt.

Parameter names are strict at the CLI boundary. If a user invocation contains an apparent typo or
near-alias, such as `grade=1,5,9-10`, the model/prompt layer must translate it to the canonical
parameter (`grades=1,5,9-10`), replay the full interpreted parameter set to the user, get explicit
confirmation, and only then invoke the CLI. The CLI must reject the non-canonical parameter directly
...
|---|---|---|---|
| `subject` | `math`, `ela` | `math` | Must resolve to `data/config/subjects/<subject>.yaml` and registered executable behavior under `src/mts/subjects/<subject>/`. `ela` generation is not yet registered; the command reports this and stops rather than guessing behavior. |
| `worksheettype` | `weekly`, `class`, `homework-4-day`, `compact-unbranded` | `weekly` | Resolves to the matching `worksheet_type_id` under `data/config/worksheet_types/`. Only `status: active` types run (`weekly-worksheet`, `class-worksheet` today). `draft`/`disabled` types are refused, and the type's `extension_readiness.blockers` are reported, unless the user explicitly accepts them for this run. |
| `gates` | `all`, `bypass all`, `bypass <gate_id>[,<gate_id>...]` | `all` | Explicit, run-scoped override of `data/config/project/base.yaml` `gates`. See Gate resolution below. Never applied silently. |
...
| `week` | `current`, a week number (e.g. `5`), or an ISO date | `current` | Resolves the `on_date` used for curriculum scope resolution against `data/config/project/base.yaml` `calendar.week_1_start` (`2026-08-17`, a Monday). See Week resolution below. |
| `publish` | `yes`, `no` | `yes` (`data/config/project/base.yaml` `publishing.default_publish`) | Whether to execute the final publish step (moving staged artifacts from `outputs-copilot/`/staging Drive into canonical `outputs/<subject>/`) automatically once Gate 5 (`publish_approval`) is recorded. `publish=no` stops after staging; artifacts remain staged pending a separate, explicit publish action. Publishing still only proceeds once `publish_approval` is recorded (approved directly or via an explicit `gates` bypass) — `publish=yes` never skips that recorded approval, it only removes the extra manual publish step afterward. |
| `deliver` | `yes`, `no` | `yes` (`data/config/project/base.yaml` `publishing.final_delivery.default_deliver`) | Whether to execute Final Delivery after publication: copying each published pair into `Week_<WEEK_OF>` under that grade's audience folder in `data/config/subjects/<subject>.yaml` `publishing.final_delivery.destinations_by_grade`. Requires `publish=yes`; `deliver=yes publish=no` is refused rather than silently downgraded, because there is nothing published to deliver. `deliver=no` ends the run at Staging. Delivery never runs before `publish_approval` is recorded, and never modifies content. |
| `difficulty` | `Low`, `Low+`, `Medium`, `Medium+`, `High`, `Very High` | `Medium+` (`data/config/project/base.yaml` `question_design.difficulty.default`) | How ambitious the difficulty ramp is across the week/day. See Question design below. |
| `diversity` | `Low`, `Low+`, `Medium`, `Medium+`, `High`, `Very High` | `Medium+` (`data/config/project/base.yaml` `question_design.diversity.default`) | How many distinct skills mix per day and how often spiral-review is injected. See Question design below. |
| `form_diversity` | `Low`, `Low+`, `Medium`, `Medium+`, `High`, `Very High` | `High` (`data/config/project/base.yaml` `question_design.form_diversity.default`) | How much a repeated skill changes mathematical action, representation, context, and response type. Applies only to active subject compatibility profiles. |
...
| `run` (resume) | an existing run id | none | Passed through unchanged to `RunLoader`/`RunWriter` to resume an in-progress run instead of starting a new one. |

## Question design

`difficulty` and `diversity` are independent, parametrizable settings on the same 6-point ordinal
scale (`data/config/project/base.yaml` `question_design.levels`), resolved via
`src/mts/subjects/<subject>/question_plan.py` `normalize_level` (accepts natural spellings like `Low+`,
`Very High`).
...
`data/config/project/base.yaml` `calendar.week_1_start` (`2026-08-17`) anchors school-year instructional week
...
`data/config/project/base.yaml` defines the five configured gates (`scope_review` through `publish_approval`,
matching `src/mts/workflow_management/gates.py` `GATES` exactly). This command resolves the `gates` parameter against
...
`data/config/project/base.yaml` `gates.bypass.non_bypassable_requirements` — Worksheet Spec persistence,
...
1. Resolve `subject` and load `data/config/project/base.yaml` plus `data/config/subjects/<subject>.yaml`. Refuse and report if the subject has no registered executable behavior under `src/mts/subjects/<subject>/`.
2. Resolve `worksheettype` against `data/config/worksheet_types/*.yaml` by `worksheet_type_id` and confirm `compatible_subjects` includes the resolved subject.
...
5. Resolve `publish` (default `yes` from `data/config/project/base.yaml` `publishing.default_publish`) and record it in the Run Manifest alongside `gates`.
6. Resolve `difficulty`, `diversity`, and `form_diversity` (defaults from `data/config/project/base.yaml` `question_design`), then generate or resolve `variation_seed` and persist it in the Run Manifest before authoring. Pass them to the subject's `build_week_plan(...)`; after authoring, run both `check_diversity_and_progression(spec)` and `check_form_diversity(spec)` as QA gates. See Question design above.
...
9. Execute the resolved subject's execution runbook directly (for `subject=math` and
   ...
   `skills/math/weekly-worksheet-execution-runbook.md` after migration),
...
11. Every Drive document this command produces must come from a persisted Spec revision, be named from `naming.<worksheet_kind>` configuration, and be stamped with provenance. Authoring content directly into a Drive document is prohibited: it produces an artifact with no Spec, no verification record, and no way to regenerate it.
...
`src/mts/subjects/math` behavior and the [skills/math/weekly-worksheet-execution-runbook.md](../skills/math/weekly-worksheet-execution-runbook.md)
workflow. Math-specific command notes live under `commands/math/`; worksheet type, effective config,
and gates are resolved here first and passed through.

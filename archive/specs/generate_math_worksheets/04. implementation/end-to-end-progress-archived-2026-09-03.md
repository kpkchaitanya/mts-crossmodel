...
| 2.1 | 2026-09-03 | Folder-structure refactor execution started | Reopened M5/M6/M7 for target `src/mts`, `tests`, and `data` migration plus a Math Weekly bypass sample run for week `2026-09-07`. |
| 2.2 | 2026-09-03 | Initial structure and sample-run validation complete | Added target `src/mts` compatibility packages, target data compatibility layout, focused tests, and offline Math Weekly bypass sample evidence for week `2026-09-07`. |
| 2.3 | 2026-09-03 | Legacy roots retired | Moved legacy transaction evidence to `data/transactions/legacy_runs`, archived old mixed/source roots under `archive/legacy-layout`, and regenerated the next-week sample run from the new active structure. |
| 2.4 | 2026-09-03 | Next-week sample rendered to staging | Updated the renderer for target `data/transactions` entity references and rendered five Math Weekly worksheet/key pairs to Google Docs staging for week `2026-09-07`. |
| 2.5 | 2026-09-03 | Seamless generate command runner added | Added `scripts/generate_worksheet.py` so command-style `key=value` invocation resolves defaults, records gate bypasses, renders staging docs, and runs delivery dry-run from the migrated structure. |
| 2.6 | 2026-09-03 | Parameter typo confirmation boundary | Made CLI parameter names strict and documented that the model/prompt layer must translate likely typos, replay the interpreted parameters, and get user confirmation before invoking code. |
| 2.7 | 2026-09-03 | Confirmed publish run stopped at delivery rate limit | Ran confirmed `grades=1,5,9-10` publish/deliver flow for week `2026-09-07`; render and publish passed, Final Delivery stopped on Google Drive `userRateLimitExceeded`, and failure evidence was recorded. |
| 2.8 | 2026-09-03 | Curriculum-driven Question Plan fix | Updated design and code so generation persists `worksheet_plan.json` and `question_plan.json`, derives Question Plan slots from resolved Weekly Curriculum/Yearly Curriculum context, and validates authored Specs against planned slot metadata. |
...
| M5 | Structure refactor complete | M5.11 implemented the target `src/mts`, `tests`, and `data` folder structure and retired legacy mixed roots after validation. |
| M6 | Migration validation complete | Target import, structure, data-layout, loader/writer, weekly lifecycle, sample-run, full root test suite, and Math migration gate passed after legacy-root retirement. |
| M7 | Complete for offline sample operation | Math Weekly sample run for week `2026-09-07` generated target transaction evidence with all gates explicitly bypassed for that run only. |
| M8 | Not complete | Governance, learning capture, final approval disposition, and optimization review remain after the migration/sample run evidence is resolved. |

### Folder Structure Refactor Checkpoint

| Work item | Evidence | Status |
|---|---|---|
| Architecture/design decision | Architecture and design define Functional Area source axis and Subject -> Grade/Course -> Cycle -> Batch -> Worksheet data axis | Complete |
| Implementation plan | `plan-and-tasks.md` M5.11 and M5.12 | In progress |
| Target source package | `src/mts` capability and subject package structure | Initial compatibility layer complete |
| Target tests | `tests/subjects/math` and target structure/data tests | Focused validation complete |
| Target data | `data/config`, `data/master`, `data/transactions` compatibility paths | Initial compatibility data complete |
| Target loaders/writers | Effective config resolver plus RunLoader/RunWriter and SpecLoader/SpecWriter | Focused validation complete |
| Legacy root retirement | `subjects`, root `config`, root `templates`, root legacy `src` folders, and root `runs` retired | Complete |
| Sample Math bypass run | Week `2026-09-07`, all gates explicitly bypassed, verification and QA still required | Generated, verified, QA checked, and rendered to Google Docs staging |

### Math Weekly Bypass Sample Run - 2026-09-07

Run: `run-2026-09-07-weekly-bypass-sample`

Mode: generated sample transaction evidence plus live Google Docs staging render. No canonical publication or Final Delivery was attempted.

| Evidence | Location | Status |
|---|---|---|
| Run manifest | `data/transactions/runs/run-2026-09-07-weekly-bypass-sample/run_manifest.json` | Complete |
| Effective config snapshot | `data/transactions/runs/run-2026-09-07-weekly-bypass-sample/effective_config.json` | Complete |
| Entity references | `data/transactions/runs/run-2026-09-07-weekly-bypass-sample/entity_references.json` | Complete |
| Worksheet transaction records | `data/transactions/subjects/math/grades/<grade>/cycles/2026-09-07/batches/weekly_math_2026_09_07_bypass/worksheets/weekly_worksheet/` | Complete for 5 worksheets |
| Worksheet and Question Plans | `worksheet_plan.json` and `question_plan.json` under each Worksheet transaction record | Complete; Spec metadata is validated against planned slots |
| Gate bypass records | Five gates per worksheet, 25 records total | Complete |
| Verification and QA | Deterministic verification and rendered-text QA remain required and passed for all sample Specs | Complete |
| Live staging render | `data/transactions/runs/run-2026-09-07-weekly-bypass-sample/rendered-artifacts.json` | Complete for 5 worksheet/key pairs |
| Final Delivery dry-run | `python scripts/deliver_weekly_worksheets.py --run-root data/transactions/runs/run-2026-09-07-weekly-bypass-sample --week-of 2026-09-07 --dry-run` | Passed for 5 grade folders; no audience files copied |
| Seamless command invocation | `python scripts/generate_worksheet.py subject=math worksheettype=weekly week=next gates="bypass all" publish=no deliver=no run=run-2026-09-07-weekly-cli` | Passed; rendered 5 worksheet/key pairs and completed delivery dry-run |
| Parameter typo boundary | `python -m pytest tests/integration/test_generate_worksheet_runner.py` | Passed 6/6; `grade=` is rejected by CLI with `Did you mean 'grades'?` |
| Confirmed prompt invocation | `python scripts/generate_worksheet.py subject=math worksheettype=weekly grades=1,5,9-10 week=2026-09-07 gates="bypass all" publish=yes deliver=yes run=run-2026-09-07-grades-1-5-9-10-publish-deliver` | Rendered and published 3 pairs; Final Delivery stopped on Google Drive rate limit after a partial Grade 1 copy |
| Delivery failure evidence | `data/transactions/runs/run-2026-09-07-grades-1-5-9-10-publish-deliver/delivery-failure.json` | Recorded; no automatic retry attempted |
| Focused automated validation | `python -m pytest tests/integration/test_weekly_math_lifecycle.py tests/test_target_structure.py tests/test_target_data_layout.py tests/subjects/math/test_target_package_imports.py tests/integration/test_math_weekly_bypass_sample.py` | Passed 10/10 |
| Loader/writer validation | `python -m pytest tests/test_target_loaders_writers.py tests/integration/test_math_weekly_bypass_sample.py` | Passed 3/3 |
| Root automated test suite | `python -m pytest tests` | Passed 51/51 |
| Post-retirement root automated test suite | `python -m pytest tests` | Passed 103/103 |
| Question Plan validation | `python -m pytest tests/integration/test_generate_worksheet_runner.py tests/integration/test_math_weekly_bypass_sample.py` | Passed 7/7; verified persisted plan chain, plan-to-spec metadata preservation, and combined Math 1/Math 2 scope files |
| Math migration gate | `python tests/math/validate_migration.py` | Passed 4/4 target suites |

| Grade/course | Student worksheet | Answer key |
|---|---|---|
| Grade 1 | https://docs.google.com/document/d/1rbOjEid48mCLqfCW4G7ZFaIrtNyWmmSfQ4dwj7s9Fvc/edit | https://docs.google.com/document/d/1xNaBJxYZ5NOST7Y2ncheq7c4GBdM-oXbksNKJrrWirc/edit |
| Grade 4 | https://docs.google.com/document/d/1ttVxMbsE1MpfjB9yiuMCn8eRhpAMKpC1ckLbhS32zg0/edit | https://docs.google.com/document/d/1GMBFOGZ_0JQL9QTz34fcFVR_IBqa8ohPFwD38fBFu60/edit |
| Grade 5 | https://docs.google.com/document/d/1HCmnLNKx48QgB6MrUF0207hLhRPHpQf8yivQyXlWDZM/edit | https://docs.google.com/document/d/1kllT_UA3mDysswVeohZVTzrJ9QpjBAdsBnM_6ec-BDs/edit |
| Grade 6 | https://docs.google.com/document/d/1J4vveDO9imf_zdnw0tzjlRlNOvAktrKaitb2cLxW24Y/edit | https://docs.google.com/document/d/1rOEHH_ev9YgDnzfXjYazDfpEUm9dy5bo8c0jSN1L5P0/edit |
| Grades 9/10 | https://docs.google.com/document/d/1XKYkENm_5YLOdIggE7k5fkcAWaH3SldwdpPItUwcPPw/edit | https://docs.google.com/document/d/1zvbvq6rL96zkDhBpveDDvBxwfR9f25FyGx5iKvgR7Yo/edit |
...
| 2026-09-03 | Started folder-structure refactor execution. | Reopened M5/M6/M7 for code/data/test migration and planned the Math Weekly bypass sample run for week `2026-09-07`. |
| 2026-09-03 | Generated Math Weekly bypass sample evidence. | Target package/data/sample tests passed 9/9 and sample run `run-2026-09-07-weekly-bypass-sample` produced 5 worksheet records with 25 explicit gate bypasses. |
| 2026-09-03 | Completed focused migration validation. | Weekly lifecycle plus target structure/data/import/sample tests passed 10/10; root test suite passed 51/51; Math migration gate passed 3/3 suites and 14 checks. |
| 2026-09-03 | Added target loaders/writers. | Effective config resolver now reads target `data/config` and `data/master`; RunLoader/RunWriter and SpecLoader/SpecWriter use `data/transactions`; focused loader/writer validation passed 3/3. |
| 2026-09-03 | Retired legacy roots and regenerated next-week sample. | Legacy transaction evidence moved to `data/transactions/legacy_runs`; old mixed/source roots moved to `archive/legacy-layout`; root tests passed 94/94; Math migration gate passed 4/4; `run-2026-09-07-weekly-bypass-sample` regenerated successfully. |
| 2026-09-03 | Rendered next-week sample to Google Docs staging. | Renderer now reads target entity references; live staging render created five worksheet/key pairs and recorded links in `rendered-artifacts.json`; root tests passed 99/99 and Math migration gate passed 4/4. |
| 2026-09-03 | Verified delivery mapping for next-week sample. | Final Delivery dry-run resolved all five grade audience folders for `Week_2026-09-07` without copying audience-facing files. |
| 2026-09-03 | Added seamless generate command runner. | `scripts/generate_worksheet.py` supports command-style parameters and the migrated Math weekly path; `run-2026-09-07-weekly-cli` rendered 5 staged worksheet/key pairs and dry-ran delivery. |
| 2026-09-03 | Added strict parameter confirmation boundary. | CLI now rejects non-canonical `grade=` and requires the model/prompt layer to confirm interpreting it as `grades=` before execution; focused runner tests passed 6/6. |
| 2026-09-03 | Ran confirmed prompt invocation with publish/deliver. | User confirmed `grade=1,5,9-10` as `grades=1,5,9-10` for week `2026-09-07`; render and publish passed for 3 pairs; Final Delivery stopped on Google Drive `userRateLimitExceeded`; failure evidence was recorded and tests passed 103/103. |
| 2026-09-03 | Fixed Question Plan generation chain. | Design now requires Yearly Curriculum -> Weekly Curriculum -> Worksheet Plan -> Question Plan -> Authored Questions -> Spec; `generate_worksheet.py` persists and validates plan records; focused tests passed 7/7, full tests passed 103/103, and migration gate passed 4/4. |

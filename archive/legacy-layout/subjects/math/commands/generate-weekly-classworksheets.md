...
Apply the `gates` resolution from `/generate-worksheet` when present; default to `config/base.yaml`
`gates` (all gates enabled) when invoked directly. Present the configured Gate 1 curriculum scope
first and stop for approval unless Gate 1 was explicitly bypassed for this run.

Apply the `publish` resolution from `/generate-worksheet` when present; default to `publish=yes`
(`config/base.yaml` `publishing.default_publish`), which publishes automatically once Gate 5 is
recorded. Pass `publish=no` to stage artifacts only and stop before the publish step.

Apply `difficulty`/`diversity` from `/generate-worksheet` when present; default to `medium_plus`
(`config/base.yaml` `question_design`). Call `MathSubjectModule.build_week_plan(...)` before authoring

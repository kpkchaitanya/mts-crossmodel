...
BASE = REPO / 'data' / 'master' / 'subjects' / 'math'
pacing = json.loads((BASE / 'curriculum/ccs-2026-2027/pacing.json').read_text())
manifest = json.loads((BASE / 'template_manifests/class-worksheet.json').read_text())
standards = json.loads((BASE / 'curriculum/nc-math/standards-cache.json').read_text())
...
def test_curriculum_cache():
    r = p0.resolve_curriculum(pacing, 'grade_6', '2026-08-24', standards=standards)
    assert r['cache_hit'] is True
    assert r['source'] == 'weekly_cache'
    assert 'NC.6.RP.1' in r['current']
    assert r['confidence'] == 'inferred'
    assert r['standards_context']['source'] == 'nc_standards_cache'
    assert r['topics'] == ['ratio concepts', 'ratio reasoning', 'factors and multiples']

    fallback = p0.resolve_curriculum(pacing, 'grade_6', '2026-08-31', standards=standards)
    assert fallback['cache_hit'] is True
    assert fallback['source'] == 'month_cache'
    assert fallback['requires_weekly_resolution'] is True
    assert fallback['topics_source'] == 'nc_standards_cache'
    assert set(fallback['topics']) == {
        'ratio concepts', 'unit rates', 'ratio reasoning', 'fraction division',
        'multi-digit computation', 'factors and multiples', 'rational numbers',
        'area', 'volume', 'coordinate plane', 'nets and surface area',
    }
...
def test_math_verifier():
    ...
    assert p0.compute('ratio', {'left': 5, 'right': 6}) == '5:6'
